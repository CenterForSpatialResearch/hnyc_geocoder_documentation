...
+ `ST Unopened [YYYY]`: Legal category for street. If the historical map shows the street as open (see legend), do not enter anything in this field (default). If the map symbolizes the street as unopened (hollow with dashed line down center; see legend), enter a `1` in this field. Note: This category does not differentiate between in-use streets and planned streets.
