...
+ `ST Unopened [YYYY]`: "Paper Streets". If the street was open on the historical basemap (see legend), do not enter anything in this field (default). If the street was closed, planned (drawn on the map) but not yet opened, enter a **1** in this field. If any part of the individual street segment is shaded/surfaced or open, for the purposes of coding this field, we will consider the entire segment `open`.
